...
const levelBtn = document.getElementById('levelBtn');
const levelCheckWrapper = document.getElementById('levelCheckWrapper');
const levelAnswerInput = document.getElementById('levelAnswerInput');
const levelSubmitBtn = document.getElementById('levelSubmitBtn');
const levelFeedback = document.getElementById('levelFeedback');

// Обработчик события на кнопку "Level Check"
levelBtn.addEventListener('click', () => {
  // Показываем поле ввода и кнопку для проверки уровня
  levelCheckWrapper.style.display = 'block';
  // Скрываем кнопку "Level Check" после её нажатия
  levelBtn.style.display = 'none';

  // Делаем так, чтобы фокус был в поле ввода
  levelAnswerInput.focus();
});

// Обработчик для кнопки "Проверить уровень"
levelSubmitBtn.addEventListener('click', () => {
  const userAnswer = levelAnswerInput.value.trim();
  
  if (userAnswer) {
    levelFeedback.textContent = `Вы ввели: ${userAnswer}`;
    levelFeedback.className = 'success'; // Для зелёной подсветки
  } else {
    levelFeedback.textContent = 'Пожалуйста, введите ваш уровень.';
    levelFeedback.className = 'error'; // Для красной подсветки
  }

  // Даем немного времени и скрываем поле ввода снова
  setTimeout(() => {
    levelCheckWrapper.style.display = 'none';
    levelBtn.style.display = 'inline-block'; // Показываем кнопку "Level Check"
    levelAnswerInput.value = ''; // Очистка поля ввода
    levelFeedback.textContent = ''; // Очищаем обратную связь
  }, 2000); // Скрываем поле через 2 секунды
});
